...
def encontrar_todos_nos(node, lista_parametros, label):
    for filho in node.children:
        lista_parametros = encontrar_todos_nos(filho, lista_parametros, label)

        if filho.label == label:
            lista_parametros.append(filho)
        
    return lista_parametros

def get_variavel(node, line, add=True):
    lista_nomes = encontrar_todos_nos(node, list(), 'ID')
    exist = False
    for nome in lista_nomes:
        if nome.children[0].label in var_list:
            if add:
                var_list[nome.children[0].label][-1][-1].append((line, node))
            exist = True
        else:
            if nome.anchestors[-1].label != 'chamada_funcao':
                message = ('ERROR', f'Erro: Variável "{nome.children[0].label}" não declarada.')
                message_list.append(message)
    return 0

def get_function(node, line, p):
    function_nomes = node.descendants[1].label
    if function_nomes in function_list:
        function_list[function_nomes][-1][-1].append((line, node))
    else:
        function_list[function_nomes] = [[function_nomes, '', 0, [], [], -1, -1, False, [(line, node)]]]
...
def p_declaracao_variaveis(p):
    ...
    arvore = encontrar_todos_nos(p.slice[-1].value, list(), 'ID')
    for no in arvore:
        nome_var = no.children[0].label 
        tipo_var = p.slice[1].value.children[0].children[0].label  
        dimensao = encontrar_todos_nos(p.slice[-1].value, list(), 'expressao') 
        nome_dimensao = []

        if len(dimensao) > 0:
            for son in dimensao:
                aux = encontrar_todos_nos(son, list(), 'numero')

                if len(aux) == 0:
                    aux = encontrar_todos_nos(son, list(), 'var')
                nome_dimensao.append((aux[-1].children[-1].children[-1].label, aux[-1].children[-1].label))

        if nome_var in var_list:
            var_list[nome_var].append([nome_var, tipo_var, len(dimensao), nome_dimensao, escopo, p.lineno(2), list()])
        else:
            var_list[nome_var] = [[nome_var, tipo_var, len(dimensao), nome_dimensao, escopo, p.lineno(2), list()]]
...
def p_cabecalho(p):
    ...
    for elemento in var_list:
        for var in var_list[elemento]:
            if inicio_linha <= var[-2] < final_linha:
                var[4] = nome_function

    # tipo das variaveis
    arvore = encontrar_todos_nos(p.slice[5].value, list(), 'RETORNA')
    for index in range(len(arvore)):
        arvore[index] = arvore[index].anchestors[-1]
    
    retorna = list()
    for no in arvore:
        tipo = 'inteiro'
        if len(encontrar_todos_nos(no, list(), 'NUM_PONTO_FLUTUANTE')) > 0:
            tipo = 'flutuante'
            ...
            for id_chamada in ids:
                ...
                if label_id in function_list:
                    if label_id == nome_function:
                        if tipo_function == 'flutuante': 
                            tipo = 'flutuante'
                    else:
                        if function_list[label_id][0][1] == 'flutuante':
                            tipo = 'flutuante'
                elif label_id in var_list:
                    for index in range(len(var_list[label_id]) - 1, -1, -1):
                        if var_list[label_id][index][4] == nome_function:
        ...
        retorna.append((tipo, no))

    if nome_function in function_list:
        if function_list[nome_function][0][-2]:
            message = ('ERROR', f'Erro: Função "{nome_function}" já declarada anteriormente.')
            message_list.append(message)
        else:
            function_list[nome_function][0] = [nome_function, tipo_function, num_var, nome_parametro, retorna, inicio_linha, final_linha, True, function_list[nome_function][0][-1]]
    else:
        function_list[nome_function] = [[nome_function, tipo_function, num_var, nome_parametro, retorna, inicio_linha, final_linha, True, []]]
...
def p_parametro(p):
    ...
    if nome_var in var_list:
        var_list[nome_var].append(
            [nome_var, tipo_var, len(dimensao), nome_dimensao, escopo, p.lineno(2), list()])
    else:
        var_list[nome_var] = [[nome_var, tipo_var, len(dimensao), nome_dimensao, escopo, p.lineno(2), list()]]
...
def p_atribuicao(p):
    ...
    p[3].parent = pai
    get_variavel(p.slice[0].value, p.lineno(2))
...
def p_chamada_funcao(p):
    ...
    if len(p) > 2:
        ...
        p[1].parent = pai
    get_variavel(p.slice[3].value, p.lineno(2))
    get_function(p.slice[0].value, p.lineno(2), p)
...
def main():
    ...
    return root, function_list, var_list, message_list
# Build the parser.
